...
The `summary()` function can be a useful tool for summarizing many kinds of objects.  The `$length` column is summarized below.  Add a line below it to summarize the whole data.frame at once.
...
You'd actually like to pull the 95% confidence interval of the difference in means so you can use it own code, without a lot of copy-pasting!  First, add a line that calls `str()` to look at the structure of the test results.
...
You can create a new column to our example data.frame, simply by assigning a new vector of data to a new column name of your choice.

Create a new column, and populate it with `"Small"` if `exampledf$length < 100`, and with `"Big"` otherwise, using the `ifelse()` function.
