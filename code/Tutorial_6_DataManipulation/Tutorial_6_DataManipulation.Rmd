---
title: "Tutorial 6 - Basic Data Manipulation"
tutorial:
...
pinksalmonratio <- read_csv("https://raw.githubusercontent.com/justinpriest/ADFG_R_Intro_Workshop/main/data/OceanAK_PinkSalmonRatio_2000-2020.csv") %>%
...
```
# learnr Tutorial 6 - Data Manipulation
...
```{r say-hello-to-your-little-friend, exercise=TRUE, exercise.eval=TRUE, exercise.setup="data-import"}
...
```{r pipe-str, exercise=TRUE, exercise.lines = 5, exercise.setup="data-import"}
str(pinksalmonratio)
...
```{r filter-2019, exercise=TRUE, exercise.eval=TRUE, exercise.lines = 5, exercise.setup="data-import"}
pinksalmonratio
```
...
```{r filter-2019mult, exercise=TRUE, exercise.eval=TRUE, exercise.lines = 5, exercise.setup="data-import"}
pinksalmonratio %>% filter(Year == 2019)
```
...
Filter the data to only show rows where there is an `NA` value in `weight_lbs`:
```{r filter-weightnas, exercise=TRUE, exercise.lines = 5, exercise.setup="data-import"}
...
Filter the data to the subset of rows where District is 101 and that excludes `weight_lbs` values that are `NA`:
```{r filter-weight_notnas, exercise=TRUE, exercise.lines = 5, exercise.setup="data-import"}
...
Did you get 154 rows? Great job!  
...
```{r filter-operator, exercise=TRUE, exercise.lines = 5, exercise.setup="data-import"}
...
```{r filter-in, exercise=TRUE, exercise.lines = 5, exercise.setup="data-import"}
...
```{r rename, exercise=TRUE, exercise.lines = 5, exercise.setup="data-import"}
...
```{r selectkeep, exercise=TRUE, exercise.lines = 5, exercise.setup="data-import"}
...
```{r selectdrop, exercise=TRUE, exercise.lines = 5, exercise.eval=FALSE, exercise.setup="data-import"}
pinksalmonratio %>% select(-)
```
...
```{r mutate-comment, exercise=TRUE, exercise.lines = 5, exercise.eval=FALSE, exercise.setup="data-import"}
...
```{r mutatenew, exercise=TRUE, exercise.lines = 5, exercise.eval=FALSE, exercise.setup="data-import"}
...
```{r mutate-weight, exercise=TRUE, exercise.lines = 5, exercise.eval=FALSE, exercise.setup="data-import"}
...
```{r mutate-if, exercise=TRUE, exercise.lines = 5, exercise.eval=FALSE, exercise.setup="data-import"}
...
```{r mutate-factor, exercise=TRUE, exercise.lines = 5, exercise.eval=FALSE, exercise.setup="data-import"}
...
```{r mutate-factorreturn, exercise=TRUE, exercise.lines = 5, exercise.eval=FALSE, exercise.setup="data-import"} 
pinksalmonratio %>% mutate(District = )




```
<div id="mutate-factorreturn-hint">
**Hint:** `pinksalmonratio %>% mutate( = as.numeric(as.character()))` 
</div>
...
quiz(
  ...
  question("You type a command in RStudio, and suddenly you can't do anything!  You notice a red stop sign in the upper right corner of the Terminal pane.  Which of the following _might_ be true? ",

    answer("Computer Over.  Virus = Very Yes."),
        answer("You totally did NOT mean to do that.  Solution: no solution.  You're doomed to wait until R finishes with whatever the heck it's doing."),
    answer("You totally did NOT mean to do that.  Solution: click the red stop sign to get your life back.", correct = TRUE),
    answer("You totally DID mean to do that!  Solution: go play outside while R finishes thinking.", correct = TRUE)
  )
)
```
