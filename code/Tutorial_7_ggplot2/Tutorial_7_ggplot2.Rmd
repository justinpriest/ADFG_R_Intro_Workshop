---
title: "Tutorial 7 - Creating charts with ggplot2"
tutorial:
...
Recall that the correct geom for a scatterplot is `geom_point()` and elements are connected by a "+"
...
### Flipper Plot - Exercise 1b 
As an aside, notice the warning above the plot:  
`Warning: Removed 2 rows containing missing values (geom_point()).`  
It is saying that two data points cannot be plotted. Most commonly this is because there are NAs in your dataset. Let's check that by clicking "Run Code". 
You don't need to write any code here, but note that we see two data points where the bill length was not measured. ggplot was just trying to let us know that there were NAs, and that's completely fine.  
```{r flipper-ex1b, exercise=TRUE, exercise.eval=FALSE}
penguins %>% 
...
*Looking good! Check out that legend that is automagically created, cool huh?*  
*Let's keep building our plot and add a few more elements. It's often important to change the shape size and transparency. These are modified by adding* `size=` *and* `alpha=` *where alpha is the percent transparency, ranging 0-1. This information only pertains to a specific geom, so that's where you'd add it.*
...
*Colors in ggplot can be a little confusing as there are lots of ways to do it. For now, let's set colors manually. Changing plot elements such as color or axes are done using the family of functions starting with * `scale_`. *To manually set colors you would add* `scale_color_manual()` *and within this add an argument for* `values=c()`*. Make sure to have as many values as color categories!*  
...
```{r lineplot-ex0, exercise=TRUE, exercise.eval=FALSE, exercise.setup="data-import"}
head()
...
```{r lineplot-ex1, exercise=TRUE, exercise.eval=FALSE, exercise.setup="data-import"}
...
```{r lineplot-ex2, exercise=TRUE, exercise.eval=FALSE, exercise.setup="data-import"}
...
```{r lineplot-ex3, exercise=TRUE, exercise.eval=FALSE, exercise.setup="data-import"}
ggplot(yearcounts, aes(x = year, y = totalcount, color = species)) +
  geom_line(size = 1.5) +
...
```{r lineplot-troubleshoot, exercise=TRUE, exercise.eval=TRUE, exercise.setup="data-import"}
